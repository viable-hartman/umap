...
L.U.Map.include({
    ...
    HIDDABLE_CONTROLS: ['zoom', 'search', 'fullscreen', 'embed', 'locate', 'measure', 'tilelayers', 'editinosm', 'datalayers', 'star'],
    ...
    star: function () {
        if (!this.options.umap_id) return this.ui.alert({content: L._('Please save the map first'), level: 'error'});
        let url = L.Util.template(this.options.urls.map_star, {'map_id': this.options.umap_id});
        this.post(url, {
            context: this,
            callback: function (data) {
                this.options.starred = data.starred;
                let msg = data.starred ? L._("Map has been starred") : L._("Map has been unstarred")
